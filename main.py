...
ser = serial.Serial('/dev/tty.usbmodem1201', 9600)  # Update with correct port
...
def login():
    ...
    password_entry = Entry(window, show="*",font=("Arial", 40,'bold'), fg="#ad8140",bg="white",insertbackground='red')
...
def main():
    ...
    def attendance_menu(): #######attendance----------------------------
        ...
        global button_image_1,ltime,name_label,name_entry,search_button,roll_number_label,attendance_table,status1,name,scan_rfid
        button_1.place_forget()
        button_2.place_forget()
        button_3.place_forget()
        button_4.place_forget()
        ...
        def read_card():
            #global carddata
            line = ser.readline().decode('utf-8').strip()  # Remove extra whitespace characters
            sensor_values = line.split(',')
            carddata = sensor_values[0]
    ...
    def register_menu(): #######--------------register----------------------------
        ...
        def scan():
            ...
            #scan_rfid=Label(window, text=generate_random_number(), font=my_font, borderwidth=0,highlightthickness=0, bg="#ad8140", fg="#1d2f71")
        ...
        def thread_function():
            ...
            while True:
                car=read_card()
                scan_rfid = Label(window, text=car, font=my_font, borderwidth=0, highlightthickness=0,bg="#ad8140", fg="#1d2f71")
                scan_rfid.place(x=642, y=340, width=328, height=66)
    ...
    def view_panel(): #######viewwwwwww----------------------------
        ...
        rfid_number_entry = Label(window, text="Scan card", font=("Arial", 25, 'bold'), bg="white", fg='#ad8140')
        ...
        def reset_quick_att_data():
            ...
            connection.commit()
            hello_label = Label(window, text="Quick Attendance Deleted", bg="green",fg="red",font=("Arial", 20))
            hello_label.place(x=813, y=150, width=600, height=75)
        ...
        def quick_att_data():
            ...
            hello_label = Label(window, text="Quick Attendance", bg="green", font=("Arial", 20))
            hello_label.place(x=813, y=150, width=600, height=75)
            connection.commit()


        def view_data():
            global canvas,rfid_number_entry
            # Get values from entry widgets
            global rows,headers,query
            rfid_number = rfid_number_entry.cget("text")
        ...
        roll_number_entry = Entry(window, font=("Arial", 25, 'bold'), bg="white", fg='#ad8140',insertbackground='red')
        ...
        month_entry = Entry(window, font=("Arial", 25, 'bold'), bg="white", fg='#ad8140',insertbackground='red')
        table.place(x=75, y=190, width=615.0, heigh=520.0)

        from_date_entry_date = DateEntry(window, width=12,insertbackground='red')
        from_date_entry_date.place(x=1043, y=460, width=157, height=25)


        to_date_entry_date = DateEntry(window, width=12,insertbackground='red')
        ...
        quick_att_btn = Button(window, text="Quick Att", font=("Arial", 25, 'bold'), fg="#1d2f71", bg='#1d2f71',command=quick_att_data)
        reset_quick_att_btn = Button(window, text="Reset Quick Att", font=("Arial", 18, 'bold'), fg="#1d2f71", bg='#1d2f71')
        ...
        def thread_function():
            global rfid_number_entry
            while True:
                car=read_card()
                rfid_number_entry = Label(window, text=car, font=("Arial", 25, 'bold'), bg="white", fg='#ad8140')
                rfid_number_entry.place(x=1023, y=263, width=223, height=59)
    ...
    def quick_attendance_menu(): #######quickattendance----------------------------
        ...
        def read_card():
            ...
            sensor_values = line.split(',')
            carddata = sensor_values[0]
            #scan_rfid = Label(window, text=carddata, font=my_font, borderwidth=0,highlightthickness=0, bg="#ad8140", fg="#1d2f71")
            #scan_rfid.place(x=642, y=340, width=328, height=66)
            print(carddata)
            return sensor_values[0]
        ...
        my_font = ('times', 50, 'bold')
        ltime = Button(window, font=my_font, borderwidth=0,highlightthickness=0, bg="#ad81ff", fg="#ad8140")
        ltime.place(x=740.0,
                 y=330.0,
                 width=595.0,
                 height=86.0)
        ...
        def current_atte(carddata):
            ...
            if result:
                ...
                cursor.execute(f"INSERT INTO quick_att (rfid,roll_no,name) VALUES (%s ,%s ,%s)",(card,roll_of_roll,name_of_roll))
                roll_number = result[0]  # Assuming roll_number is the first column in the query result
                roll_number_label.config(text=f"{roll_number}\n{name_of_roll}")
                status1 = Label(window, text="PRESENT", font=my_font, borderwidth=0,highlightthickness=0, bg="green", fg="white")
                status1.place(x=733.0, y=700.0, width=610.0, height=100.0)
                update_attendance_table()
                connection.commit()
        ...
        search_button = Button(window, text="Search", command=current_atte)
        roll_number_label = Label(window, text="Next",font=my_font, borderwidth=0,highlightthickness=0, bg="#ad8140", fg="#ffffff")
        head_label = Label(window, text="Quick Attendance", font=my_font, borderwidth=0, highlightthickness=0, bg="#ad8140",fg="#1d2f71")
        status1 = Label(window, text="----", font=my_font, borderwidth=0,highlightthickness=0, bg="#ad8140", fg="white")
        status1.place(x=733.0, y=700.0, width=610.0, height=100.0)
        # Arrange widgets using grid layout
        name_label.grid(row=0, column=0, padx=5, pady=5)
    ...
    def update_panel():
        global button_image_1, ltime, Canvas, canvas , next_btn ,rfid_numb_ent,rfid_numb
        ...
        def update_name():
            ...
            def upd_name():
                # Check if any textbox is empty, if so, ignore
                global query
                rfid_number_up = rfid_numb.cget("text")
        ...
        def delete_user():
            ...
            def del_user():
                ...
                rfid_number_del = rfid_numb.cget("text")
        ...
        def search_user():
            ...
            rfid_number_sr = rfid_numb.cget("text")
        ...
        def thread_function():
            ...
            while True:
                car=read_card()
                rfid_numb = Label(window, text=car, font=("Arial", 25, 'bold'),fg="#ad8140", bg='black')
                rfid_numb.place(x=424, y=285, width=226, height=59)
                return car
        ...
        rfid_numb_ent = Label(window,text="",font=("Arial", 25, 'bold'),fg="#ad8140", bg='black')
